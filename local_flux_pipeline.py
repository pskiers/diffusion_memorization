...
class LocalFluxPipeline(FluxPipeline):
    def get_pred_denoised_image(
        ...
        **kwargs,
        ...
        if "intervention_strenght" in kwargs and "token_intervention" in kwargs and "token_intervention_pos" in kwargs:
            intervention = kwargs["token_intervention"].to(prompt_embeds_init.device)
            positions = torch.tensor(kwargs["token_intervention_pos"], device=prompt_embeds_init.device)
            strenghts = torch.tensor(kwargs["intervention_strenght"], device=prompt_embeds_init.device)
            batch_idx = torch.arange(intervention.shape[0], device=prompt_embeds_init.device)
            prompt_embeds_init[batch_idx, positions] += intervention * strenghts.unsqueeze(dim=0)
        ...
        for i, t in enumerate(timesteps):
            ...
            noise_pred = self.transformer(
                ...
                encoder_hidden_states=prompt_embeds_init,
                txt_ids=text_ids,
            ...
            sigma = self.scheduler.sigmas[sigma_idx]
            current_sigma = sigma

            latents = latents - current_sigma * noise_pred  # pred clean image
            ...
            return image, prompt_embeds_init
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                noise_pred = self.transformer(
                    ...
                    guidance=guidance,
                    pooled_projections=pooled_prompt_embeds,
                    encoder_hidden_states=prompt_embeds, # NOTE
                    txt_ids=text_ids,
                    img_ids=latent_image_ids,
                    joint_attention_kwargs=self.joint_attention_kwargs,
                    return_dict=False,
