...
class LocalStableDiffusionXLPipeline(StableDiffusionXLPipeline):
    ...
    def get_text_cond_grad(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # check if we are doing the LID calculations
                if i in target_steps:
                    ...
                    if method == "flipd":
                        ...
                        def score_fn(x):
                            x_in = torch.cat([x, x]) if self.do_classifier_free_guidance else x
                            in_prompt = torch.cat([dummy_prompt_embeds.repeat(x.shape[0], 1, 1),single_prompt_embeds.repeat(x.shape[0], 1, 1)]) if self.do_classifier_free_guidance else single_prompt_embeds.repeat(num_images_per_prompt, 1, 1)
                            n_pred = self.unet(
                                x_in,
                                t,
                        ...
                        ) if self.do_classifier_free_guidance else torch.norm(noise_pred, p=2)
                    ...
                    # just do the normal denoising step
                    with torch.no_grad():
                        noise_pred = self.unet(
                            latent_model_input,
                            t,
    ...
    def decode_latent(self, latents, output_type="pil"):
    ...
    def get_pred_denoised_image(
        ...
        prompt_embeds_init = prompt_embeds_init.to(device)
        prompt_embeds_init = prompt_embeds_init.clone().detach()
        prompt_embeds_init.requires_grad = True
        prompt_embeds_init.retain_grad()
        ...
        if self.do_classifier_free_guidance:
            prompt_embeds = torch.cat([negative_prompt_embeds, prompt_embeds_init], dim=0)
            add_text_embeds = torch.cat([negative_pooled_prompt_embeds, add_text_embeds], dim=0)
            add_time_ids = torch.cat([negative_add_time_ids, add_time_ids], dim=0)
        else:
        ...
        for i, t in enumerate(timesteps):
            ...
            latents = self.decode_latent(latents, output_type=output_type)

            return latents, prompt_embeds_init
